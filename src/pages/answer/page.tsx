                                ...
                                <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-admin-panel/40 p-4 border border-admin-border rounded-xl mt-12">
                                    ...
                                    <div className="flex items-center gap-4 font-mono">
                                        <div className="text-center">
                                            <p className="text-[10px] text-admin-text-dim uppercase leading-none mb-1">Current_Index</p>
            ...
            <div className="p-5 flex flex-col xl:flex-row gap-6">
                ...
                <div className="xl:w-72 space-y-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="overflow-hidden">
                            <h4 className="text-xs font-black text-admin-text uppercase truncate">
                                {session.user_id?.nickname || "REDACTED_SUBJECT"}
                            </h4>
                            <p className="text-[9px] font-mono text-admin-text-dim truncate">
                    ...
                    <div className="grid grid-cols-2 gap-2 font-mono">
                        <div className="p-2 bg-admin-bg/50 border border-admin-border rounded flex flex-col items-center">
                            <span className="text-[7px] text-admin-text-dim uppercase font-bold">Archetype</span>
                            <span className="text-[10px] font-black text-admin-accent italic">{session.finalPsychotype}</span>
                        </div>
                        <div className="p-2 bg-admin-bg/50 border border-admin-border rounded flex flex-col items-center">
                            <span className="text-[7px] text-admin-text-dim uppercase font-bold">Vote</span>
                            <span className="text-[10px] font-black text-admin-primary">{session.geminiVote}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 space-y-4">
                    <div className="bg-admin-bg/40 p-4 rounded-xl border border-admin-border/50 relative">
                        ...
                        <p className="text-[8px] font-black text-admin-primary uppercase mb-2">Gemini_Decision_Reasoning:</p>
                        <p className="text-[11px] text-admin-text leading-relaxed italic opacity-80">
                    ...
                    <div className="grid grid-cols-3 md:grid-cols-6 gap-4">
                        {Object.entries(session.percentageScores).map(([key, val]) => (
                            <div key={key} className="space-y-1.5">
                                <div className="flex justify-between text-[7px] font-black uppercase text-admin-text-dim">
                ...
                <div className="xl:w-48 flex flex-row xl:flex-col justify-between items-center xl:items-end border-t xl:border-t-0 xl:border-l border-admin-border/30 pt-4 xl:pt-0 xl:pl-6">
                    <div className="text-right">
                        <p className="text-[8px] font-black text-admin-text-dim uppercase">Sync_Date</p>
                        <p className="text-[10px] font-mono text-admin-text">{new Date(session.created_at).toLocaleDateString()}</p>
                    </div>
                    <button
                        onClick={() => setShowRaw(!showRaw)}
                        className="px-4 py-2 bg-admin-panel border border-admin-border text-[9px] font-black text-admin-primary rounded uppercase hover:bg-admin-primary hover:text-black transition-all"
                    >
                ...
                <div className="bg-black/40 border-t border-admin-border p-5 grid grid-cols-2 md:grid-cols-5 gap-3 animate-in slide-in-from-top duration-300">
                    {session.answers.map((ans, i) => (
                        <div key={ans._id} className="p-2 border border-admin-border/30 rounded flex items-center justify-between bg-admin-panel/20">
                            <span className="text-[8px] font-bold text-admin-text-dim">Q{i + 1}</span>
                            <span className="text-[10px] font-mono text-admin-primary">SEQ_{ans.selectedOptionSequence}</span>
                        </div>
                    ))}
                </div>
            )}
...
function StatsMini({ label, val, color = "text-admin-text" }: { label: string, val: any, color?: string }) {
    return (
        <div className="bg-admin-panel/50 border border-admin-border p-4 rounded-xl flex justify-between items-center">
            <span className="text-[9px] font-black text-admin-text-dim uppercase tracking-widest">{label}</span>
