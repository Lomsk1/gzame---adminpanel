...
export function SessionCard({
    ...
    session: QuestDailyTypes["data"][number];
    onInspect: () => void;
}) {
    const { t } = useAdminT();
    ...
    const timeRemaining = (date: Date) => {
        ...
        if (diff <= 0) return t("dailyQuests.session.expired");
                ...
                <div className="flex justify-between items-start">
                    <div className="space-y-1">
                        <div className="flex items-center gap-2 text-admin-text font-black text-[12px] uppercase tracking-tighter">
                            ...
                            {t("dailyQuests.session.uid")}:{" "}
                            <span className="text-admin-primary font-mono">{session.user_id.slice(-10)}</span>
                        </div>
                ...
                <div className="grid grid-cols-1 gap-2">
                    {session.quest_set.map((quest, idx) => {
                        ...
                        return (
                            <div
                                key={quest._id}
                                className={`flex items-center justify-between p-3 border transition-all duration-300 ${isDone ? "bg-admin-accent/5 border-admin-accent/30" : "bg-black/20 border-admin-border/50"}`}
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p
                                            className={`text-[11px] font-black uppercase tracking-tight ${isDone ? "text-admin-text" : "text-admin-text-dim"}`}
                                        >
                                            {quest.title.en}
                                        </p>
                ...
                <div className="flex justify-between items-center pt-3 border-t border-admin-border/30">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-mono text-admin-text-dim uppercase tracking-widest font-bold">
                            {t("dailyQuests.session.synced", { count: completionCount })}
                        </span>
                    </div>
                    <button
                        onClick={onInspect}
                    ...
                    >
                        <Terminal size={10} /> {t("dailyQuests.session.inspectPayload")}
                    </button>
                </div>
            </div>
        </GlassCard>
