...
export const LevelEditorDrawer = ({ config, onClose, onSave, isSubmitting }: Props) => {
    const { t } = useAdminT();
    const [formData, setFormData] = useState<Partial<LevelData>>(
        config || {
            level: 0,
            exp_required: 0,
            is_active: true,
    ...
    const footer = (
        ...
        >
            <span className="relative z-10">
                {isSubmitting ? t("drawer.commitInProgress") : t("drawer.commitData")}
            </span>
    ...
    return (
        <AdminDrawerShell
            ...
            title={config ? t("levels.editor.editTitle") : t("levels.editor.newTitle")}
            footer={footer}
        >
            <div className="grid grid-cols-2 gap-4">
                <AdminInput
                    label={t("levels.editor.levelId")}
                    type="number"
                ...
                />
                <AdminInput
                    label={t("levels.editor.expRequired")}
                    type="number"
            ...
            <div className="p-4 border border-admin-border/50 bg-admin-panel/20 rounded space-y-4">
                <p className="text-[10px] font-black text-admin-warning uppercase tracking-[0.2em]">
                    {t("levels.editor.rewardPayload")}
                </p>
                <AdminInput
                    label={t("levels.editor.energyBonus")}
                    type="number"
            ...
            <div className="flex items-center gap-3 p-3 border border-admin-border/20 bg-white/5 rounded-sm">
                ...
                <label htmlFor="active-toggle" className="text-[10px] font-black uppercase text-admin-text cursor-pointer select-none">
                    {t("levels.editor.activeInProduction")}
                </label>
            </div>
        </AdminDrawerShell>
    );
